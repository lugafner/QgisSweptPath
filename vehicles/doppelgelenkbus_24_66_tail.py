...
# Vehicle Doppelgelenkbus 24.66m Hess Trolleybus (End section)
from ..vehicle import Vehicle

class Doppelgelenkbus2466Tail(Vehicle):
    def __init__(self):
        super().__init__()

        # Set up all needed parameters
        self._vehicle_name = "Doppelgelenkbus 24.66m Tail"
        # Vehicle geometry based on template from canton of lucerne

        # Body
        self._body_length = 6.015  # meter
        self._body_width = 2.55  # meter

        # Chassis
        self._front_axle_ref_pos = -0.83  # meter from front
        self._rear_axle_ref_pos = 3.355  # meter from front
        ...
        self._connection_point = 0.0  # meter from front (negative = point is in front of vehicle)
        ...
        # Graphics
        self._symbol = "./doppelgelenkbus_24_66_tail.svg"
        self._symbol_size_x = 6.845  # SVG symbol size x in QGIS style units (usually meters)
